...
def define_flags():
  """Defines flags for the script."""
  # Check if flags are already defined. If so, skip redefining them.
  if not hasattr(FLAGS, 'model_file'): # This check ensures that flags are defined only once
    flags.DEFINE_string("model_file", None,
                        "Path and file name to the TFLite model file.")
    flags.DEFINE_string("label_file", None, "Path to the label file.")
    flags.DEFINE_string("export_directory", None,
